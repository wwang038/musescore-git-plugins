...
"# ---- coloring -----------------------------------------------------",
"",
"# Add-DiffMark: charCode avoids encoding issues when writing the PS1 file.",
"# atEnd=$true places the mark after the last note (for the closing marker).",
"function Add-DiffMark($doc, $meas, $charCode, $r, $g, $b, $atEnd) {",
"    $voice = $meas.SelectSingleNode('voice')",
"    if (-not $voice) { return }",
"    $st = $doc.CreateElement('StaffText')",
"    $col = $doc.CreateElement('color')",
"    $col.SetAttribute('r',$r); $col.SetAttribute('g',$g)",
"    $col.SetAttribute('b',$b); $col.SetAttribute('a',255)",
"    $st.AppendChild($col) | Out-Null",
"    $tn = $doc.CreateElement('text')",
"    $tn.InnerText = [char]$charCode",
"    $st.AppendChild($tn) | Out-Null",
"    if ($atEnd) { $voice.AppendChild($st) | Out-Null }",
"    else {",
"        $first = $voice.FirstChild",
"        if ($first) { $voice.InsertBefore($st, $first) | Out-Null }",
"        else { $voice.AppendChild($st) | Out-Null }",
"    }",
"}",
"",
...
"                            $lastOrigAnchor = $mO",
"                            if (-not $firstOrigMeas) { $firstOrigMeas = $mO }",
"                            $copyNodes.Add($imp)",
"                        }",
"                    }",
"                    'delete' {",
...
"                            if (-not $firstOrigMeas) { $firstOrigMeas = $measO[$op.oi] }",
"                            $copyNodes.Add($rp)",
"                        }",
"                    }",
"                    'insert' {",
...
"            $firstCopyNode = $copyNodes | Where-Object { $_ -ne $null } | Select-Object -First 1",
"            $copyAnchor = $lastOrigAnchor",
"            foreach ($cn in $copyNodes) {",
"                if ($cn -ne $null) { Insert-After $stO $cn $copyAnchor; $copyAnchor = $cn }",
"            }",
"            # Add boundary marks on the first staff only:",
"            #   9654 = ▶  start of diff block (first orig measure)",
"            #   8214 = ‖  transition from orig to copy (first copy measure)",
"            #   9664 = ◀  end of diff block (after last note of last copy measure)",
"            if ($sid -eq $commonIds[0]) {",
"                $markStart = if ($firstOrigMeas) { $firstOrigMeas } else { $firstCopyNode }",
"                if ($markStart)    { Add-DiffMark $ox $markStart    9654 30 100 200 $false }",
"                if ($firstCopyNode){ Add-DiffMark $ox $firstCopyNode 8214 30 100 200 $false }",
"                if ($copyAnchor -and $copyAnchor -ne $blockStartAnchor) { Add-DiffMark $ox $copyAnchor 9664 30 100 200 $true }",
"            }",
